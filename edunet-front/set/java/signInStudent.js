$(document).ready(function () {
    ...
    function signInStudent() {
        $('#alertSuccess').hide();
        $('#alertError').hide();
        var studentUser = $('#studentUser').val();
        var studentPsw = $('#studentPsw').val();
        ...
        $.ajax({
            ...
            url: "http://127.0.0.1:9090/students",
            data: JSON.stringify({ username: studentUser, password: studentPsw, firstName: studentName, lastName: studentLastName, email: studentEmail, dateBirth: studentBirth }),
            dataType: 'json',
        }).done(function (data) {
            $('#studentUser').val('');
            $('#studentPsw').val('');
    ...
    function validateStudentInputs() {
        ...
        var fechaNacimiento = $('#studentBirth').val();
